...
const ScoringModel = require('../models/scoringModel'); // Ensure this model exists and exports necessary methods
...
const ScoringAlgorithm = {
   ...
   */
  async scoreProduct(productData, datasetStats) {
    try {
      // Fetch current weights from the database
      const weights = await ScoringModel.getCurrentWeights(); // Ensure this method returns a { criterion: weight, ... } object
  ...
  /**
   * Retrieve dataset statistics for normalization.
  ...
  async getDatasetStats() {
    try {
      ...
      return {
        maxSearchVolume: maxSearchVolume.maxSearchVolume || 1, // Prevent division by zero
        maxSalesRank: maxSalesRank.maxSalesRank || 1,
        maxCompetitorCount: maxCompetitorCount.maxCompetitorCount || 1,
        maxShippingCost: maxShippingCost.maxShippingCost || 1,
        maxReturnRate: maxReturnRate.maxReturnRate || 1,
        maxSeasonalityVariation: maxSeasonalityVariation.maxSeasonalityVariation || 1,
      };
    } catch (error) {
